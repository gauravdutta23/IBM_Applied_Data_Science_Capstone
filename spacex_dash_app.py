...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        if entered_site == 'site1':
            ...
            filtered_df1 = filtered_df[['Launch Site', 'class']]
            fig = px.pie(filtered_df1, 
                         names='class', 
                         title='Total Successful Launches for Site {}'.format(site_name))
            return fig
...
def get_scatter_chart(entered_site, selected_payload):
    min_value = selected_payload[0]
    max_value = selected_payload[1]
    if entered_site == 'ALL':
        ...
        fig.update_layout(xaxis_range=[min_value - 250, max_value + 250])
        ...
        if entered_site == 'site1':
            ...
            fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class',
                             color="Booster Version Category", 
                             title='Correlation between Payload and Success for Site {}'.format(site_name),
                             width=1400, height=400)
            fig.update_layout(xaxis_range=[min_value - 250, max_value + 250])
